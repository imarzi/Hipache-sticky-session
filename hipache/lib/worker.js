...
var get_cookies = function(request) {
  ...
  return cookies;
        ...
        if (get_cookies(req)['stickyBackend'] === undefined) {
		...
		var headers = {
                "Set-Cookie": "stickyBackend="+proxy.target.host + ":" + proxy.target.port
           	};
		res.writeHead(code, headers);
		...
		var stickyBackendAddress = get_cookies(req)['stickyBackend'];
		var stickyBackendAddressTab = stickyBackendAddress.split(':');
		var stickyHost = stickyBackendAddressTab[0];
		var stickyPort = stickyBackendAddressTab[1];
		res.timer.startBackend = Date.now();
                var proxy = new httpProxy.HttpProxy({
                    target: {
                        host: stickyHost,
